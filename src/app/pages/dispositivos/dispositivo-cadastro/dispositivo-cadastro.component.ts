...
export class DispositivoCadastroComponent implements OnInit {
    ...
    dispositivoAtualJaCadastrado = false;
    ...
    constructor(
        private router: Router, private dispositivoService: DispositivoService,
        private filialService: FilialService, private toastService: ToastService,
    ...
    onCreate(): void {
        this.dispositivoService.create(this.form).subscribe(
            {
                next: data => {
                    window.localStorage.setItem(Constantes.DISPOSITIVO_CHAVE, data.chave === undefined ? '' : data.chave);
